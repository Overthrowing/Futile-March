...
function Player() {
    ...
    this.push = () => {
        ...
        const minDist = this.body2 + boulder.size;
        const canPush = distToBoulder < 3.5 && distToBoulder > minDist - 0.5 && dotProduct > 0.3 && keyDown["w"] && this.onGround();

        if (canPush && this.pushCooldown <= 0) {
            this.isPushing = true;
            boulder.applyPush(times(normalize([toBoulder[0], 0, toBoulder[2]]), 2.0));
